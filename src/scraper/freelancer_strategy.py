"""Freelancer.com scraping strategy.

Fetches gig listings from multiple Freelancer.com RSS category feeds.
"""
...
class FreelancerStrategy(BaseScrapeStrategy):
    """Concrete strategy for scraping Freelancer.com gig listings."""
    ...
    async def fetch(self) -> List[RawJobListing]:
        ...
        async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
            for i, url in enumerate(feed_urls):
                ...
                logger.info("Fetching Freelancer feed: %s", url)
                try:
                    ...
                    logger.error("Failed to fetch Freelancer feed %s: %s", url, e)

        logger.info("Freelancer: fetched %d listings", len(listings))
        return listings
    ...
    def _parse_item(self, item: ET.Element) -> RawJobListing | None:
        try:
            ...
            logger.warning("Failed to parse Freelancer item: %s", e)
            return None
